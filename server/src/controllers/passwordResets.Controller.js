  ...
  export const resetPasswordByAdmin = async (req, res) => {
    try {
      ...
      await sendPasswordResetEmail(user.email, user.username, password);
  ...
  export const handleDeleteResetRequest = async (req, res) => {
  try {
    const { request_id } = req.params;
    const existing = await resetModel.getResetRequestById(request_id);
    if (!existing) {
      return res.status(404).json({ success: false, message: "Reset request not found." });
    }

    const deleted = await resetModel.deleteResetRequest(request_id)
    return res.status(200).json({
      success: true,
      message: "Reset request deleted successfully.",
      data: deleted
    });
